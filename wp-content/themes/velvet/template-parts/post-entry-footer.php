<?php
	...
	do_action( 'velvet_before_post_entry_footer' );
?>
	...
	<ul class="entry-footer-list">
			...
			<?php if ( is_single() and get_the_category_list() ) : ?>
				<li class="post-categories">
					<?php
					printf( '<span>%s</span> %s', esc_html__( 'in ', 'velvet' ), get_the_category_list( esc_html__( ', ', 'velvet' ) ) );
					?>
				</li>
			<?php endif; ?>
		...
		<?php if ( ! is_single() and ! post_password_required() and ( comments_open() or get_comments_number() ) ) : ?>
			<li class="comments-link">
				...
				<?php
				comments_popup_link(
					esc_html__( 'Leave a response', 'velvet' ),
					esc_html__( '1 response', 'velvet' ),
					esc_html__( '% responses', 'velvet' ) );
				?>
			</li>
		<?php endif; ?>
		...
		<?php if ( velvet_option( 'show-share-button', FALSE, TRUE ) ) : ?>
			<li class="share-link">
							...
							<li class="post-share-dropdown-wrapper">
								<div class="dropdown">
									<button class="dropdown-toggle more-share"
										...
										<?php esc_html_e( 'SHARE', 'velvet' ) ?>
									</button>
									<ul class="dropdown-menu">
										<?php if ( velvet_option( 'share-button', 'twitter', TRUE ) ) : ?>
											<!--Twitter-->
											<li>
												<a class="btn-social twitter"
												   href="http://twitter.com/home?status=Reading:<?php the_permalink(); ?>"
												   title="<?php esc_html_e( 'Share this post on Twitter!', 'velvet' ); ?>"
												   target="_blank"><i
														class="zmdi zmdi-twitter"></i></a>
											</li>
										<?php endif; ?>

										<?php if ( velvet_option( 'share-button', 'facebook', TRUE ) ) : ?>
											<!--Facebook-->
											<li>
												<a class="btn-social facebook"
												   href="http://www.facebook.com/sharer.php?u=<?php the_permalink(); ?>&amp;t=<?php the_title(); ?>"
												   title="<?php esc_html_e( 'Share this post on Facebook!', 'velvet' ); ?>"
		...
		<?php echo edit_post_link( '<i class="fa fa-pencil"></i>' . esc_html__( 'Edit Post', 'velvet' ), '<li class="edit-link">', '</li>' ) ?>

		<?php do_action( 'velvet_post_entry_footer' ); ?>
	</ul>
<?php do_action( 'velvet_after_post_entry_footer' );