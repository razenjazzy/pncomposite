	...
	*/
	public function bfd_user_defined_colour_scheme()
	{
		...
		if (isset( $bfd_options['bfd-hover-colour-picker'] ) && $bfd_options['bfd-hover-colour-picker'] != 'default' ) {
			...
			// Echo out the CSS override rule.
			echo '<style> .bfd-download-btn:hover, .bfd-download-meta:hover > p{color:' . $bfd_hover_colour .'!important;} .bfd-download-btn:hover{border: 1px solid ' . $bfd_hover_colour . ' !important;} .bfd-single-download-btn:hover{color:' . $bfd_hover_colour .' !important;}</style> ';
		}
	}
	...
	public function bfd_single_download_shortcode($bfd_atts)
	{
		...
		if ( 'inline' == $bfd_args['format'] ) :
			...
			// Return the data to the user
			 ...
			 {
			 	return '<a href=" ' . esc_url($bfd_download_id ) . ' " class="bfd-inline-download" title=" ' . esc_attr__( $bfd_download_title, $this->plugin_name ) .' "  target="_blank"> ' . esc_attr__( $bfd_download_title, $this->plugin_name ) .' <em class="bfd-inline-filetype"> - ' . esc_attr__( $bfd_filetype, $this->plugin_name ) . '</em> </a>';
			 }
			 else
			 {
