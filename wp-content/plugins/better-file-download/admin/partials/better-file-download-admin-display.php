...
<div class="wrap">
	...
	<div id="bfd-tabs">
		...
		<div id="bfd-shortcodes">
			...
			<p><?php _e('The download category shortcode accepts two optional parameters, <em>show_count</em> & <em>latest</em>. As with the individual download shortcode the <em>show_count</em> parameter may be set to true if you wish to show the number of times each file in the category has been downloaded. The <em>latest</em> shortcode is used to limit the number of downloads for a category that you wish to display. For example, If you would like to display all the available downloads in a category with the slug "april-publications" and also display the download count <em>but</em> limit the number of downloads displayed to the most recent 5 downloads, the shortcode would look like this <code>[download_category category="april-publications" show_count="true" latest="5"]</code>', $this->plugin_name); ?></p>
