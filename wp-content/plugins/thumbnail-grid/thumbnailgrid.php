...
Plugin URI: http://www.shooflysolutions.com/featured-image-thumbnail-grid-for-wordpress/
...
/*
Copyright (C) 2013-2019 Shoofly Solutions (NomadCoder)
...
/**
 * thumbnailgrid_handler function.
 * handle thumbnail arguments, create grid
 * @access public
 * @param mixed $atts
 * @return void
 */
	...
	class thumbnailgrid
{
	...
	function defaults($atts)
	{
		...
		$atts= shortcode_atts( array(
				...
				'link_title' => '',              //default link title to posttitle. also can use imagecaption,  imagealt
			), $atts  );
	...
	function getSettings(&$atts)
	{
		...
		$settings->link_title = $link_title;
	...
	/**
	 * thumbnailgrid_function function.
	 *
	 * @access public
	 * @param mixed $atts (default: NULL)
	 * @return void
	...
	function thumbnailgrid_function($atts=NULL) {
		...
		$image_alt = get_post_meta( $thumb->image_id, '_wp_attachment_image_alt', true);

		$titletype = $settings->link_title == 'imagealt' || $settings->link_title == 'imagecaption'  ? $settings->link_title : '';
		$thumb->title = get_the_title();
		$thumb->alt = $image_alt != '' ? $image_alt : get_the_title();
		switch ($titletype) {
			case 'imagealt':
				$thumb->link_title = $thumb->alt;
				break;
			case 'imagecaption':
				$thumb->link_title = get_the_title( $thumb->image_id );
				break;
			default:
				$thumb->link_title = $thumb->title;
		}
	...
	public function theThumbnail($settings, $thumb)
	{
		...
		$link = "href='{$thumb->permalink}' title='{$thumb->link_title}'  $target";

		if ($thumb->image_url)
			$thumbnail = "<img alt='{$thumb->link_title}' $settings->imageclass src='{$thumb->image_url}' {$settings->imagestyle}/>";
		else
