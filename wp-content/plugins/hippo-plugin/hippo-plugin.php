<?php
	/**
	 * Plugin Name: Velvet Theme Plugin
	 * Description: Velvet Theme Helper Plugin
	 * Version: 1.0.0
		...
		function hippo_plugin_settings_link( $links ) {
			...
			if ( is_plugin_active( HIPPO_PLUGIN_BASENAME ) ) {
				$action_links = apply_filters( 'hippo_plugin_action_links', array(
					'settings' => sprintf( '<a href="' . esc_url( apply_filters( 'hippo_plugin_settings_url', admin_url( 'admin.php?page=%1$s' ) ) ) . '" title="' . esc_attr__( '%1$s Theme Settings', 'hippo-plugin' ) . '">' . esc_html__( '%1$s Settings', 'hippo-plugin' ) . '</a>', HIPPO_THEME_NAME ),
				) );
		...
		function hippo_plugin_row_meta( $links, $file ) {
			...
			if ( $file == HIPPO_PLUGIN_BASENAME and is_plugin_active( $file ) ) {
				$row_meta = apply_filters( 'hippo_plugin_row_meta', array(
					'docs'    => sprintf( '<a target="_blank" href="' . esc_url( apply_filters( 'hippo_plugin_docs_url', 'http://goo.gl/rJeXiG' ) ) . '" title="' . esc_attr__( 'View %1$s Theme Documentation', 'hippo-plugin' ) . '">' . esc_html__( '%1$s Theme Documentation', 'hippo-plugin' ) . '</a>', HIPPO_THEME_NAME ),
