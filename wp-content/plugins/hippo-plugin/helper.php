<?php
	...
	if ( ! function_exists( 'hippo_plugin_delete_cookie' ) ):
		function hippo_plugin_delete_cookie( $name ) {
			unset( $_COOKIE[ $name ] );
			hippo_plugin_set_cookie( $name, NULL, time() - YEAR_IN_SECONDS );
	...
	if ( ! function_exists( 'hippo_plugin_theme_option_get_preset' ) ):
		function hippo_plugin_theme_option_get_preset( $suffix = '' ) {
			...
			if ( ! function_exists( 'session_start' ) or ! function_exists( 'hippo_plugin_set_session' ) ) {
			...
			if ( isset( $_GET[ 'reset-hippo-preset' ] ) ) {
				hippo_plugin_delete_session( $session_name );
			...
			if ( ! empty( $require_preset ) ) {
				...
				hippo_plugin_set_session( $session_name, $require_preset );
			} elseif ( empty( $require_preset ) and hippo_plugin_get_session( $session_name ) ) {
				// Check current on-fly preset on session and return session value.
				$current = hippo_plugin_get_session( $session_name );
			} else {
